...
export const binaryTree: ShapeDefinition = {
  id: "tree",
  ...
  generate: (counter: number = 0): Geometry => {
     ...
     * 直交する軸の周りにベクトルを回転させる
     */
    const rotate = (v: Vector3, axis: Vector3, angle: number): Vector3 => {
    ...
    const addBranch = (startIndex: number, frame: any, length: number, depth: number) => {
      ...
      const buildNextFrame = (newDir: Vector3) => {
