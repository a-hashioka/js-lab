 ...
 * @param {number} counter - 生成するポイントの数。
 * @returns {Geometry} {vertices, faces, hideVertices}
 */
export const lorenz: ShapeDefinition = {
  id: "lorenz",
  ...
  formulas: [
    {
      label: "Lorenz System",
      eq: "\\begin{cases} \\dot{x} = \\sigma (y - x) \\\\ \\dot{y} = x (\\rho - z) - y \\\\ \\dot{z} = xy - \\beta z \\end{cases}",
    },
    {
      label: "Chaotic Parameters",
      eq: "\\sigma=10 \\\\ \\rho=28 \\\\ \\beta=8/3",
    },
  ],
  generate: (counter: number = 3000): Geometry => {
    const v: Vector3[] = [];
    let x = 0.1,
      y = 0,
      z = 0;
