...
const generateHoneycomb = (rows = 3, cols = 3, height = 0.2, scale = 0.2): Geometry => {
  const allVerts: Vector3[] = [];
  const allFaces: number[][] = [];

  const addHexPrism = (cx: number, cy: number, r: number, h: number) => {
...
export const honeycomb: ShapeDefinition = {
  id: "honeycomb",
  title: "Honeycomb",
  hideVertices: true,
  desc: [
  ...
  generate: (_counter: number): Geometry => generateHoneycomb(3, 3, 0.2, 0.2),
};
