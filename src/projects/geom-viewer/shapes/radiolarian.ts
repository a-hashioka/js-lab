...
const generateRadiolarian = (subdivisions = 2, scale = 0.8): Geometry => {
  let geo: any = { vertices: ICO_VERTICES, faces: ICO_FACES };
  
  const subdivideWithHoles = (geometry: any) => {
    const { vertices, faces } = geometry;
    const nextFaces: number[][] = [];
    const nextVertices: Vector3[] = [...vertices];
    const cache = new Map<string, number>();

    const getMid = (p1: number, p2: number) => {
...
export const radiolarian: ShapeDefinition = {
  id: "radiolarian",
  ...
  generate: (_counter: number): Geometry => generateRadiolarian(3, 0.8),
};
