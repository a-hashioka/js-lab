...
import { normalize, vec, subdivide } from "../utils.js";
 ...
 * @param {Object} center - 中心座標 {x, y, z}。
 * @param {string} type - 'SUN', 'PLANET', 'MOON' のいずれか。
 * @param {number} iterations - 細分化の回数。
 * @param {Object} camera - カメラの回転状態（太陽の肢減光計算用）。
 * @returns {Object} 頂点、面、および色情報のオブジェクト。
 */
const createBody = (
  radius,
  center = { x: 0, y: 0, z: 0 },
  ...
  geo.faces.forEach((face) => {
    ...
    if (type === "SUN") {
      ...
      const y1 = normal.y * cosY - normal.z * sinY;
...
export const orrery = {
   ...
   * @param {Object} state - ビューアーの状態（カメラ回転などの取得用）。
   * @returns {Object} 生成されたジオメトリデータ。
   */
  generate: (frame = 0, state = {}) => {
    const vertices = [];
    const faces = [];
    const faceColors = [];
    const edgeColors = [];
    ...
    const addBody = (bodyData) => {
      ...
      vertices.push(...v);
      faces.push(...f.map((face) => face.map((i) => i + offset + 1)));
    ...
    // 2. 惑星と衛星の設定
    const getScaledDist = (au) => 0.18 + Math.log(au + 1) * 0.5;
